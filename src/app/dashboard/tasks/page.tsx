...
export const metadata = { title: 'Tarefas & Agenda — Luknos' }

export default async function TasksPage({ searchParams }: { searchParams?: { view?: string } }) {
  const { allowedPages } = await requirePageAccess('/dashboard/tasks')
  ...
  const canSeeAgenda = allowedPages === null
    || allowedPages.some(p => p === '/schedules' || '/schedules'.startsWith(p + '/'))
