...
export function ScheduleModal({ schedule, defaultDate, defaultQuote, onClose, onSuccess }: ScheduleModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={onClose}>
      ...
      <div className="relative bg-gradient-card rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        <div className="sticky top-0 rounded-t-2xl bg-gradient-navy px-6 py-4 flex items-center justify-between z-10">
          <h2 className="text-base font-semibold text-white">{isEdit ? 'Editar agendamento' : 'Novo agendamento'}</h2>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div ref={partnerRef}>
            ...
            {partnerId ? (
              <div className="flex items-center gap-2 bg-surface-secondary border border-surface-border rounded-lg px-3 py-2 mt-1">
